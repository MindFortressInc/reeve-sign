...
describe('finalizeFieldFileUpload', () => {
  ...
  it('re-validates the FINAL object after copy and rejects if it is missing (avoids trusting the copy blindly)', async () => {
    headS3File.mockResolvedValueOnce({ exists: true, size: 1024, contentType: 'application/pdf' }); // tmp HEAD
    copyS3File.mockResolvedValueOnce(undefined);
  ...
  it('rejects if the final object HEAD reports different metadata than the tmp object (race/corruption guard)', async () => {
    headS3File.mockResolvedValueOnce({ exists: true, size: 1024, contentType: 'application/pdf' }); // tmp HEAD
    copyS3File.mockResolvedValueOnce(undefined);
